...
_log_ch_use = "Using change history table {database_name}.GENERAL_LEDGER_RAW.{table_name} " \
  + "(last altered {last_altered})"
_log_ch_create = "Created change history table {database_name}.GENERAL_LEDGER_RAW.{table_name}"
_err_ch_missing = "Unable to find change history table {database_name}.GENERAL_LEDGER_RAW.{table_name}"
...
class SnowflakeSchemachangeSession:
  ...
  _q_ch_metadata =  "SELECT CREATED, LAST_ALTERED FROM FINANCE_GENERAL_LEDGER_DEV.INFORMATION_SCHEMA.TABLES" \
    + " WHERE TABLE_SCHEMA = REPLACE('GENERAL_LEDGER_RAW','\"','') AND TABLE_NAME = replace('{table_name}','\"','')"
  _q_ch_schema_present = "SELECT COUNT(1) FROM FINANCE_GENERAL_LEDGER_DEV.INFORMATION_SCHEMA.SCHEMATA" \
    + " WHERE SCHEMA_NAME = REPLACE('GENERAL_LEDGER_RAW','\"','')"
  _q_ch_ddl_schema = "GENERAL_LEDGER_RAW"
  _q_ch_ddl_table = "CREATE TABLE IF NOT EXISTS FINANCE_GENERAL_LEDGER_DEV.GENERAL_LEDGER_RAW.{table_name} (VERSION VARCHAR, " \
  ...
  _q_ch_r_checksum = "SELECT DISTINCT SCRIPT, FIRST_VALUE(CHECKSUM) OVER (PARTITION BY SCRIPT " \
    + "ORDER BY INSTALLED_ON DESC) FROM FINANCE_GENERAL_LEDGER_DEV.GENERAL_LEDGER_RAW.{table_name} WHERE SCRIPT_TYPE = 'R' AND " \
    + "STATUS = 'Success'"
  _q_ch_fetch ="SELECT VERSION FROM FINANCE_GENERAL_LEDGER_DEV.GENERAL_LEDGER_RAW.{table_name} WHERE SCRIPT_TYPE = 'V' ORDER" \
    + " BY INSTALLED_ON DESC LIMIT 1"
  _q_sess_tag = "ALTER SESSION SET QUERY_TAG = '{query_tag}'"
  _q_ch_log = "INSERT INTO FINANCE_GENERAL_LEDGER_DEV.GENERAL_LEDGER_RAW.{table_name} (VERSION, DESCRIPTION, SCRIPT, SCRIPT_TYPE, " \
